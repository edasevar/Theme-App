...
export class NavigationProvider {
    private static elementDatabase: Map<string, ThemeElementInfo> = new Map();

    public static async showElementExamples(
        ...
        const panel = vscode.window.createWebviewPanel(
            'themeElementExamples',
            `Theme Element: ${property}`,
    ...
    private static getElementInfo(property: string): ThemeElementInfo {
    ...
    private static initializeElementDatabase(): void {
...
// Comments and plain text use this color`
    ...
    public static searchElements(query: string): ThemeElementInfo[] {
        // Initialize database if not done
        if (this.elementDatabase.size === 0) {
            this.initializeElementDatabase();
        }
        
        const results: ThemeElementInfo[] = [];
    ...
    public static getElementsByCategory(category: string): ThemeElementInfo[] {
        // Initialize database if not done
        if (this.elementDatabase.size === 0) {
            this.initializeElementDatabase();
        }
    ...
    public static getRelatedElements(property: string): ThemeElementInfo[] {
        const element = this.getElementInfo(property);
        if (!element || !element.relatedProperties) {
            return [];
        }
        
        const related: ThemeElementInfo[] = [];
    ...
    public static getAllElements(): ThemeElementInfo[] {
        // Initialize database if not done
        if (this.elementDatabase.size === 0) {
            this.initializeElementDatabase();
        }
    ...
    public static getElementCount(): number {
        // Initialize database if not done
        if (this.elementDatabase.size === 0) {
            this.initializeElementDatabase();
        }
    ...
    public static generateDemoTheme(baseColor: string): any {
        const demo: any = { colors: {} };
    ...
    private static generateColorVariations(baseColor: string): any {
    ...
    private static async navigateToElement(property: string): Promise<void> {
        const elementInfo = this.getElementInfo(property);
        ...
        if (elementInfo.vsCodeCommand) {
            try {
                ...
                vscode.window.showInformationMessage(
                    `Navigated to show: ${property}. Look for the element described as "${elementInfo.example}"`
                );
            } catch (error) {
            ...
            const action = await vscode.window.showInformationMessage(
                `${property} is located: ${elementInfo.location}. ${elementInfo.example}`,
                'Open Settings', 'Show Command Palette'
            );
    ...
    private static async showInSettings(property: string): Promise<void> {
        try {
    ...
    private static async testColorOnElement(property: string, color: string): Promise<void> {
        // Temporarily apply the color to test it
        const config = vscode.workspace.getConfiguration();
        const workbenchConfig = config.get('workbench.colorCustomizations') as any || {};
        ...
        const action = await vscode.window.showInformationMessage(
            `Testing ${property} with color ${color}. Do you want to keep this change?`,
            'Keep', 'Revert'
        );
    ...
    private static getExamplesHtml(elementInfo: ThemeElementInfo): string {
        return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    ...
    <div class="container">
        ...
        <div class="section">
            ...
            <div class="related-properties">
                ${elementInfo.relatedProperties.map(prop => 
                    `<button class="related-prop" onclick="openRelated('${prop}')">${prop}</button>`
                ).join('')}
            </div>
        </div>
        ` : ''}
