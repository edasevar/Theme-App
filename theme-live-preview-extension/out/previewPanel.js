...
class PreviewPanel {
    constructor(extensionUri, themeExtractor) {
        this.themeExtractor = themeExtractor;
        this._disposables = [];
        this._currentCSS = '';
        this._currentThemeName = 'No Theme Loaded';
        this._extensionUri = extensionUri;
        this._panel = vscode.window.createWebviewPanel(PreviewPanel.viewType, 'Theme Live Preview', vscode.ViewColumn.Beside, {
            enableScripts: true,
    ...
    navigateToThemeItem(themeItem) {
        this._panel.webview.postMessage({
            command: 'highlightThemeItem',
            themeItem: themeItem
        });
    }
    async handleColorPicker(currentColor, property) {
    ...
    async applyColorToVSCode(property, color) {
        try {
            ...
            vscode.window.showInformationMessage(`Applied ${property}: ${color} to VS Code theme`, 'Open Settings').then(selection => {
...
PreviewPanel.viewType = 'themeLivePreview';
