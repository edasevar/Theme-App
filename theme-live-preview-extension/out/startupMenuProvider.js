...
class StartupMenuProvider {
    ...
    static async makeNewTheme(context) {
        try {
    ...
    static async useCurrentTheme(context) {
        try {
            ...
            for (const extension of extensions) {
                if (extension.packageJSON.contributes?.themes) {
                    ...
                    const matchingTheme = themes.find((theme) => theme.label === currentTheme || theme.id === currentTheme);
                    if (matchingTheme) {
    ...
    static async chooseTheme(context) {
        try {
            ...
            // Show theme picker
            const selectedTheme = await vscode.window.showQuickPick(themes.map(theme => theme.label), {
    ...
    static async insertJsonData(context) {
        try {
    ...
    static async insertVsixFile(context) {
        try {
            const fileUri = await vscode.window.showOpenDialog({
                canSelectFiles: true,
                canSelectFolders: false,
    ...
    static async insertCssFile(context) {
        try {
            const fileUri = await vscode.window.showOpenDialog({
                canSelectFiles: true,
                canSelectFolders: false,
    ...
    static async randomizeTheme(context) {
        try {
    ...
    static async generateTheme(context, description) {
        try {
    ...
    static async convertCSSToTheme(cssContent) {
        const themeData = {
    ...
    static generateAIGuidedTheme(description) {
        ...
        if (hasBlue)
            baseColors.accent = '#007acc';
        else if (hasGreen)
            baseColors.accent = '#4ec9b0';
        else if (hasRed)
            baseColors.accent = '#f44747';
        else if (hasPurple)
            baseColors.accent = '#c586c0';
        if (isHighContrast) {
