...
exports.ValueEditorProvider = void 0;
const vscode = __importStar(require("vscode"));
class ValueEditorProvider {
    static async showValueEditor(context, options) {
        return new Promise((resolve) => {
            ...
            const panel = vscode.window.createWebviewPanel('valueEditor', `Edit: ${options.property}`, { viewColumn: vscode.ViewColumn.Beside, preserveFocus: false }, {
                enableScripts: true,
    ...
    static generateSuggestions(property, currentValue) {
        const suggestions = [];
        // Color suggestions based on property type
        if (property.includes('background')) {
            suggestions.push('#1e1e1e', '#2d2d30', '#252526', '#3c3c3c', '#464647', '#ffffff', '#f3f3f3', '#eeeeee', '#e5e5e5');
        }
        else if (property.includes('foreground')) {
            suggestions.push('#d4d4d4', '#cccccc', '#ffffff', '#969696', '#858585', '#333333', '#2d2d30', '#1e1e1e', '#000000');
        }
        else if (property.includes('accent') || property.includes('highlight')) {
            suggestions.push('#007acc', '#0e639c', '#1177bb', '#569cd6', '#4fc1ff', '#f44747', '#ff6b6b', '#4ec9b0', '#c586c0', '#d7ba7d');
        }
        else {
            // General color suggestions
            suggestions.push('#1e1e1e', '#2d2d30', '#252526', '#3c3c3c', '#007acc', '#d4d4d4', '#cccccc', '#ffffff', '#f44747', '#4ec9b0');
        }
        // Filter out current value if it exists
        return suggestions.filter(s => s !== currentValue);
    }
    static getValueEditorHtml(options) {
...
exports.ValueEditorProvider = ValueEditorProvider;
ValueEditorProvider.activeEditors = new Map();
//# sourceMappingURL=valueEditorProvider.js.map